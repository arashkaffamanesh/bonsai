...
# Deploy k3s master on k3s-master
...
for WORKER in ${WORKERS}; do multipass exec ${WORKER} -- /bin/bash -c "curl -sfL https://get.k3s.io | K3S_TOKEN=${K3S_TOKEN} K3S_URL=${K3S_NODEIP_MASTER} sh -"; done
sleep 10

echo "############################################################################"
# multipass exec k3s-master -- bash -c "sudo kubectl get nodes"
...
for WORKER in ${WORKERS}; do kubectl label node ${WORKER} node-role.kubernetes.io/node= ; done
